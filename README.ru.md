...
5.  **Генерация изображений с помощью CLI-клиента (`./imagine run`):**
    ...
    **Пример: Базовое преобразование текста в изображение (txt2img) через CLI**
    ```bash
    ./imagine run "a photo of an astronaut riding a horse on mars, epic, cinematic, detailed" -m dreamshaper_8 -w 768 -h 512 -n 25 -g 7.0 --sampler 'dpm++ 2m'
    ```
    Это сохранит изображение в формате `PNG` и файл метаданных `JSON` в текущей директории.

    **Example: Преобразование изображения в изображение (img2img) через CLI**
    ```bash
    ...
    **Example: Потоковая передача промежуточных шагов через CLI**
    ```bash
    ./imagine run "кошка в крошечной шляпке, акварельная живопись" -s 5 -c 1
...
6.  **Взаимодействие с API (POST-запрос к `/generate`):**
    ...
    *   `clip` (целое число, необязательно): Пропуск слоев CLIP. Указывает, сколько слоев пропустить в текстовом кодировщике CLIP. Более высокие значения (например, 2-12) могут привести к более "сырому" или менее "перестилизованному" выводу, в зависимости от модели. По умолчанию: `None` (по умолчанию для модели или 1).
