...
1.  **Откройте в браузере:** Перейдите в директорию `ui/` вашего клонированного репозитория и откройте файл `imagine-ui.html` в вашем любимом веб-браузере.
    ```
...
3.  **Настройте параметры и сгенерируйте:** Заполните желаемые промпты и настройте другие параметры. Нажмите кнопку "Generate". *Или, чтобы загрузить ранее сгенерированные настройки, используйте кнопку "Загрузить настройки из JSON" в секции "Конфигурация" и выберите JSON-файл, созданный утилитой `./imagine run`.*
...
3.  **Управление моделями:**
    Imagine ожидает, что ваши файлы моделей Stable Diffusion (`.safetensors` или `.ckpt`) будут расположены в специально выделенной директории.
...
5.  **Генерация изображений с помощью CLI-клиента (`./imagine run`):**
    ...
    *   `-m, --model MODEL`: **Имя** SD-модели для использования (например, `dreamshaper_8`). По умолчанию: `dreamshaper_8`.
...
6.  **Взаимодействие с API (POST-запрос к `/generate`):**
    ...
    *   `model` (строка, необязательно): **Имя** Stable Diffusion модели для использования (например, `dreamshaper_8`). Это имя должно соответствовать файлу `.safetensors` или `.ckpt` (без расширения) в настроенной директории моделей сервера. По умолчанию: `dreamshaper_8`.
