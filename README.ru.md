...
4.  **Настройте параметры сервера (Устройство, Точность):**
    Вы можете настроить вычислительное устройство и точность с плавающей запятой непосредственно через аргументы командной строки при запуске сервера.
    *   `--device` или `-d`: Укажите `'cpu'`, `'cuda'` (для NVIDIA GPU) или `'mps'` (для Apple Silicon) для вычислений вашей модели. По умолчанию `'cuda'`.
    *   `--full_prec` или `-f`: Используйте этот флаг для включения `torch.float32` (полная точность). По умолчанию используется `torch.float16` (половинная точность), что может обеспечить более быстрый вывод и меньшее использование VRAM на совместимом оборудовании (например, NVIDIA GPU), но может повлиять на качество.

5.  **Запустите сервер:**
    ```bash
    python imagine.py --host '0.0.0.0' -p 5000 -d cuda --full_prec # Пример для CUDA с float32
    # python imagine.py # Запускается со значениями по умолчанию (cuda, float16)
    ```
...
6.  **Отправьте POST-запрос на `/generate`:**
    ...
    *   `seed` (целое число, необязательно): Случайное начальное число для воспроизводимости. По умолчанию: случайное целое число, представленное в виде строки.
...
**Примечание:** Этот скрипт `imagine-cli.py` является отдельной утилитой для генерации изображений, которая может взаимодействовать с сервером `imagine.py`.

### Описание

Простой, но мощный скрипт командной строки для генерации изображений на основе текстовой подсказки с использованием библиотеки `diffusers`. Он предоставляет расширенный формат вывода для полной воспроизводимости и бесшовной интеграции с веб-интерфейсом Imagine UI.
...
options:
  ...
  -o, --output OUTPUT   Имя файла для сохранения изображения и сопровождающего JSON (по умолчанию - на основе временной метки, например, `20231027_153045.png` и `20231027_153045.json`).
...
Скрипт `imagine-cli.py` по умолчанию генерирует файл JSON вместе со сгенерированным изображением. Этот JSON-файл содержит все параметры, использованные для генерации (поле `meta`), и само сгенерированное изображение в формате base64 (поле `out`).
...
*   `meta`: Содержит все параметры (подсказка, модель, размеры, шаги и т.д.), которые были отправлены на сервер Imagine для этой конкретной генерации. Это обеспечивает полную отслеживаемость того, как было создано изображение.
...
По умолчанию скрипт сохранит два файла в текущую директорию:
*   Файл изображения `.png` с именем, основанным на подсказке или временной метке (например, `my_prompt_12345.png` или `20231027_153045.png`).
...
Он идеально подходит для быстрой экспериментов, тестирования параметров или демонстрации возможностей сервера.
...
2.  **Настройте адрес сервера:** В секции "Server & Model" проверьте или установите "Imagine Server Address" (по умолчанию `http://localhost:5000`).

3.  **Настройте параметры и сгенерируйте:** Заполните желаемые промпты и настройте другие параметры. Нажмите кнопку "Generate".
    *Или, чтобы загрузить ранее сгенерированные настройки, используйте кнопку "Загрузить настройки из JSON" в секции "Конфигурация" и выберите JSON-файл, созданный утилитой `imagine-cli.py`.*
...
> **Примечание о CORS:** Если вы столкнетесь с проблемами подключения (например, ошибками "Failed to fetch"), это может быть связано с политиками Cross-Origin Resource Sharing (CORS). Убедитесь, что ваш браузер разрешает локальный доступ к файлам для внешних ресурсов, или рассмотрите возможность запуска простого локального веб-сервера для обслуживания директории `ui/` (например, `python3 -m http.server 8000` из корневой директории проекта `imagine`, затем перейдите по адресу `http://localhost:8000/ui/imagine-ui.html`).
...
# Рекомендуется включить аргументы сервера, такие как устройство и точность, здесь.
...
*   Убедитесь, что символическая ссылка `/usr/bin/imagine-server` корректно указывает на ваш **серверный скрипт** (`imagine.py`).
*   Строка `ExecStart` в `imagine.service` должна указывать желаемые аргументы сервера (например, `-d cuda --full_prec`).
