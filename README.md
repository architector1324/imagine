...
1.  **Download the Imagine Binary:**
    Grab the latest pre-built executable for your operating system from the [Releases page](https://https://github.com/architector1324/imagine/releases).
...
3.  **Model Management:**
    ...
    *   **Listing Available Models:**
        To see which models are available to the server, use the `list` command:
        ```bash
        ./imagine list
        ```
        This will output a list of model names found in the configured models directory (e.g., `dreamshaper_8`).
    *   **Downloading Models:** You can download Stable Diffusion models from sources like Civitai or Hugging Face and place them in your models directory (e.g., `~/.imagine/models/dreamshaper_8.safetensors`).
...
5.  **Generate Images using the CLI Client (`./imagine run`):**
    The `imagine` binary itself acts as a powerful command-line client to interact with a running Imagine server.
    ...
    *   `-n, --steps NUM_STEPS`: Number of inference steps.
    *   `-g, --guidance GUIDANCE`: Guidance scale.
    *   `-d, --strength STRENGTH`: Denoising strength (only if `--img` provided).
    *   `-c, --clip CLIP`: Clip skip (integer, usually 1-12).
    *   `--sampler SAMPLER`: SD Sampler (see list below).
    *   `-i, --img IMG`: Input image file path.
    *   `-f, --hires HIRES`: High Resolution fix scale (e.g., `1.5`).
    *   `--seed SEED`: Random seed for reproducibility.
    *   `--neg NEG`: Negative prompt.
    *   `-s, --stream STREAM`: If set to an integer `N > 0`, intermediate images will be streamed every `N` steps, continuously updating the output `.png` file.
    ...
    **Example: Basic Text-to-Image (txt2img) via CLI**
    ...
    ```
    This will save a `PNG` image and a `JSON` metadata file in the current directory.
...
6.  **Interact with the API (POST request to `/generate`):**
    ...
    *   `clip` (int, optional): Clip skip. Specifies how many layers to skip in the CLIP text encoder. Higher values (e.g., 2-12) can lead to more "raw" or less "over-stylized" output, depending on the model. Default: `None` (model's default or 1).
...
The `./imagine run` command (CLI client) defaults to generating a JSON file alongside the generated image. This JSON file contains all the parameters used for the generation (`meta` field) and the generated image itself in base64 format (`out` field).
...
By default, the client will save two files to the current directory:
